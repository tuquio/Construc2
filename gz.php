 ...
 *
 * WebMechanic 03.10.2011 added CACHE_FOLDER
 */
ob_start();

/*
 * Subfolder within DocumentRoot, writable. Contains the
 * gzipped ressources
 */
define('CACHE_FOLDER', 'tmp/gzip/');
...
if ( $compress ) {
    $gz_uri = CACHE_FOLDER . $src_uri . ".gz";
    ...
    if ( !file_exists( $gz_uri ) ) {
        if ( !file_exists( CACHE_FOLDER . $uri_dir ) )
            mkdir_r( CACHE_FOLDER . $uri_dir );
        $error = false;
